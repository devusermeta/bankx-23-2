"""
Account Agent A2A Microservice - FastAPI Server
...
from agent_handler import get_account_agent_handler, cleanup_handler
from config import (
    A2A_SERVER_PORT,
    A2A_SERVER_HOST,
    ACCOUNT_AGENT_NAME,
...
AGENT_CARD = {
    "name": "Account Agent",
    "description": (
        "Specialized banking agent for account management operations. "
        "Handles account balance inquiries, account details, payment method information, "
        "and transaction limits. Uses MCP tools to access customer account data."
    ),
...
async def lifespan(app: FastAPI):
    ...
    logger.info("🚀 Starting Account Agent A2A Microservice...")
    ...
    # Initialize handler
    handler = await get_account_agent_handler()
    logger.info("✅ Account Agent Handler initialized")
    
    logger.info(f"✅ Account Agent A2A Server ready on {A2A_SERVER_HOST}:{A2A_SERVER_PORT}")
    ...
    # Cleanup on shutdown
    logger.info("🛑 Shutting down Account Agent A2A Microservice...")
...
# Create FastAPI app
app = FastAPI(
    title="Account Agent A2A Server",
    description="Banking account specialist agent exposed via A2A protocol",
    version="1.0.0",
    lifespan=lifespan,
)
...
async def root():
    ...
    return {
        "service": "Account Agent A2A Microservice",
        "version": "1.0.0",
...
if __name__ == "__main__":
    # Run the server
    logger.info(f"Starting Account Agent A2A Server on {A2A_SERVER_HOST}:{A2A_SERVER_PORT}")
