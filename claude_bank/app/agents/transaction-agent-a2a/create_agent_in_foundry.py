"""
One-time script to create TransactionAgent in Azure AI Foundry WITHOUT MCP tools
...
from config import AZURE_AI_PROJECT_ENDPOINT, TRANSACTION_AGENT_MODEL_DEPLOYMENT, TRANSACTION_AGENT_NAME

# Load environment variables
load_dotenv(override=True)

# Read agent instructions from prompts file
PROMPTS_PATH = Path(__file__).parent / "prompts" / "transaction_agent.md"
with open(PROMPTS_PATH, "r", encoding="utf-8") as f:
    AGENT_INSTRUCTIONS = f.read()


async def create_transaction_agent():
    """Create TransactionAgent in Azure AI Foundry WITHOUT MCP tools"""
    
    if not AZURE_AI_PROJECT_ENDPOINT or not TRANSACTION_AGENT_MODEL_DEPLOYMENT:
        raise ValueError(
    ...
    print("=" * 80)
    print("🚀 Creating TransactionAgent in Azure AI Foundry")
    print("=" * 80)
    print(f"Project Endpoint: {AZURE_AI_PROJECT_ENDPOINT}")
    print(f"Model Deployment: {TRANSACTION_AGENT_MODEL_DEPLOYMENT}")
    print(f"Agent Name: {TRANSACTION_AGENT_NAME}")
    print("=" * 80)
    ...
    async with (
        ...
        agent_definition = PromptAgentDefinition(
            model=TRANSACTION_AGENT_MODEL_DEPLOYMENT,
        ...
        print(f"   TRANSACTION_AGENT_VERSION={agent.version}")
        ...
        print("\n3. Start the Transaction Agent A2A server:")
        print("   cd claude_bank/app/agents/transaction-agent-a2a && python main.py")
        print("=" * 80)
...
if __name__ == "__main__":
    try:
        asyncio.run(create_transaction_agent())
